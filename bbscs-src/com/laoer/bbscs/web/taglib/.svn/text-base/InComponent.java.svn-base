...
public class InComponent extends Component {
	...
	public boolean start(Writer writer) {
		...
		if (this.getType().equalsIgnoreCase("commend")) {//C:/Users/LongMin/Downloads/bbscs_8_0_3/build/web/include/
		...
		if (this.getType().equalsIgnoreCase("userlit")) {
			File commendFile = new File(BBSCSUtil.getIncludePath() + "UserLit.html");//C:/Users/LongMin/Downloads/bbscs_8_0_3/build/web/include/
			String commendlist = "";
			try {
				commendlist = FileUtils.readFileToString(commendFile, Constant.CHARSET);//是空白但不是null
				if (commendlist == null) {
					commendlist = "";
				}
			} catch (IOException ex) {
			...
			this.write(writer, commendlist);//输出空白
			return result;
		}
